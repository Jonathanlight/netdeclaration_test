NTD.t_identif_20160101 = {
  ...
  onSelectModePaiement: function(elt) {
    ...
    if (elt && $F(elt)) {
      ...
      if ($$("[name='t_identif_20160101_opts[payment]']").length > 0)
        $$("[name='t_identif_20160101_opts[payment]']").first().setValue($F(elt));
    }
  },

  showPaiement: function(amount) {
    var initialize = true;
    $('table-t_identif_20160101_select_paiement').show();
    $('table-t_identif_20160101_select_paiement').select('input[name=select_paiement]').each(function(select_paiement) {
      select_paiement = $(select_paiement);
      if (select_paiement.checked) {
        NTD.t_identif_20160101.onSelectModePaiement(select_paiement);
        initialize = false;
      }
    });

    if (initialize) {
      NTD.t_identif_20160101.onSelectModePaiement(null);
      $('select_paiement_tlra').click(null);
    }

    if ($F('declaration-status') != 0) return;

    var total_amount = Number($F('t_identif_20160101_HA')) + Number($F('t_identif_20160101_HB')) + Number($F('t_identif_20160101_HC'));
    if (total_amount != amount) {
      $('t_identif_20160101_HA').setValue(amount);
      $('t_identif_20160101_HB').setValue(0);
      $('t_identif_20160101_HC').setValue(0);
    }
    $$('.buttons-container').last().removeClassName('hide-on-last-page');
  },

  hidePaiement: function() {
    $('table-t_identif_20160101_select_paiement').hide();

    NTD.t_identif_20160101.onSelectModePaiement(null);

    if ($F('declaration-status') != 0) return;

    $('t_identif_20160101_HA').setValue('');
    $$('.buttons-container').last().addClassName('hide-on-last-page');
  },

  onSelectOGA: function(elt) {
    $('t_identif_20160101_OGA').setValue($F(elt));
    $('t_identif_20160101_OGA').onchange();
  }
