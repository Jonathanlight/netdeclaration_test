...
//      this.iframe.observe("readystatechange", this.onLoad.bindAsEventListener(this)); 
    }
    else {
      this.onLoad();
    }
    ...
    for (key in actions) {
      ...
      img = new Element("img", {"src": "/app/core/pix/richtext/" + key + ".gif", 
                                "class": "action", 
                                "width": 10, 
                                "height": 10});
    ...
    elt.src = "/app/core/pix/richtext/src.gif";
...
//    this.onRegister();
  ...
  onLoad: function() {
    ...
    link.type = "text/css";
    link.rel = "stylesheet";
    link.href = "/app/core/styles/stylesheet.css";
    link.media = "screen";
    link.async = true;
...
//    this.doc.execCommand("insertBrOnReturn", false, false);
  ...
  onLinkInsert: function(h, link) {
    if (h["link-url"].length < 1) {
      return alert("Attention ! Veuillez préciser le lien.");
    }
    this.dialog.close();
    var html = "";
    if (link == null) {
    ...
    if (h["link-type"] === "download") {
      ...
      if (!h["link-url"].include("@")) {
        return alert("Attention ! L’adresse email semble invalide.");
      }
