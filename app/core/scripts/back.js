...
Kwo.Class.FilePanel = Class.create({
  ...
  onCallback: function(res) {
    ...
    for (i = 0; i < l; ++i) {
      ...
      elt.file = file;
      html = "";
      html += '<img src="/app/core/pix/mime/' + file["ico"] +'" />';
      html += file["name"];
      elt.update(html);
      if (file["is_folder"] == 1) {
...
Kwo.Class.ItemPicker = Class.create(Kwo.Dialog, {
  ...
  onBind: function() {
	...
	if (that.elt.hasAttribute("data-caller-model")) {
	  this.setAttribute("data-caller-model", that.elt.getAttribute("data-caller-model"));
	}
	if (that.elt.hasAttribute("data-caller-item")) {
	  this.setAttribute("data-caller-item", that.elt.getAttribute("data-caller-item"));
	}
...
Kwo.USR = Class.create(Kwo.Dialog, {
  ...
  onDisplay: function() {
    Kwo.exec("/back/community/user.summary", this.args,
             {container: this.support});
  }
...
kwo.manager.Dashboard = Class.create({
  ...
  initialize: function(elt) {
    ...
    this.overview.observe("click", this.onRefine.bindAsEventListener(this));
  },
  ...
  onPeriodSelect: function(evt) {
    evt.stop();
    var elt = evt.element();
    if (!(elt.tagName == "IMG" || elt.tagName == "A")) return ;
    this.overview.update('<div class="wrapper"></div>');
    this.precision.update('<div class="wrapper"></div>');
  ...
  onRefine: function(evt) {
    evt.stop();
    var elt = evt.element();
    if (!elt.hasAttribute("data-action")) return ;
    var action = "/back/core/" + elt.getAttribute("data-action");
  ...
  initialize: function(elt) { 
    ...
    if (this.isItemEditable()) {
      this.wrapper.down(".item-attach-button").observe("click", function (evt) {
        ...
        var ed = new Kwo.Class.ItemEditor(this); 
        ed.onAfterClose = that.onRefresh.bind(that)
      });
    }
    else {
      this.wrapper.down(".item-attach-button").observe("click", function (evt) {
    ...
    this.rows.on("click", "SPAN.content", function (evt, elt) { 
      evt.stop();
      if (!that.isEditable() && !that.isItemEditable()) return ;
      var li = elt.up("LI");
      if (that.isEditable()) {
        var args = that.getArgs();                                                     
      ...
      if (that.isItemEditable()) {
        var ed = new Kwo.Class.ItemEditor(li);
        ed.onAfterClose = that.onRefresh.bind(that);
      }
    });
...
//    elt.remove();
  ...
  isItemEditable: function() {
    return this.rows.hasClassName("item-editable");
  },
  ...
  onAfterDisplay: function() { 
    ...
    /*Sortable.create(ul.id,
                    {"handle": "handle", 
                     "scroll": this.rows,
                     "onUpdate": this.onSort.bind(this)});*/
    var ref = new ScrollSort(ul, this.onSort.bind(this));
  },
...
kwo.ux = {
  ...
  onInputReset: function(elt) {
    ...
    if (elm.hasClassName("elem-image")) {
      ...
      elm.down("IMG.elt-image").src = "/app/core/pix/unknown.gif";
    }
  },
  ...
  onItemEdit: function(elt) {
    elt = $(elt);
    var ed = new Kwo.Class.ItemEditor(elt);
...
kwo.manager.Discount = Class.create({
  "id": null,
  "type": null,
  "elt": null,
  ...
  onClick: function(evt) {
    ...
    if (elt.hasClassName("condition-edit") || elt.up().hasClassName("condition-edit")) {
      ...
      Kwo.exec("/back/shop/discount.condition.edit", 
      ...
      Kwo.exec("/back/shop/discount.action.edit", {"id": this.id},
      ...
      Kwo.exec("/back/shop/discount.condition.remove", 
               {"id": this.id, "condition_id": elt.getAttribute("data-id")},
               {"confirm": true,
                "callback": this.onRefresh.bind(this)});
    }
  ...
  onConditionSave: function(evt) {
    ...
    args["id"] = this.id;
    Kwo.exec("/back/shop/discount.condition.save", args,
             {"callback": function (resp) {
               if (Kwo.hasError(resp)) return Kwo.error(resp);
               that.onRefresh();
             }})
  ...
  onActionSave: function(evt) {
    ...
    var args = evt.element().form.serialize(true);
    args["id"] = this.id;
    Kwo.exec("/back/shop/discount.action.save", args,
             {"callback": function (resp) {
               if (Kwo.hasError(resp)) return Kwo.error(resp);
               that.onRefresh();
             }})
  ...
  onRefresh: function() {
    Kwo.exec("/back/shop/discount.composer", {"id": this.id},
             {"container": this.elt});
  }
...
var ScrollSort = Class.create({
  ...
  initialize: function (elt, callback, opts) {
    ...
    //$(this.elt).select('li').invoke('observe', 'dragover', this.over_callback);
  ...
  drag: function (event) {
    ...
    //$(this.from).stopObserving('dragover', this.over_callback);
  ...
  over: function (event) {
    ...
    //$(this.from).observe('dragover', this.over_callback);
